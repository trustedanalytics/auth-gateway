 ...
 *      CuratorFramework object should be created before each request because of potential change
 *      of kerberos ticket under the hood.
 *
 */
public class KerberosfulZookeeperClient implements ZookeeperClient {
    ...
    public void createZnode(String znodePath, String content, String username,
        ...
        try {
            ...
            LOGGER.info(
                "Caught: '" + e.getMessage() + "' while creating node. Ensure ACLs are correct:");
    ...
    public void removeUserFromAcl(String znodePath, String username) throws Exception {
        List<ACL> acls = curatorClient.getACL().forPath(pathOps.makePath(znodePath));
        acls = acls.stream()
            .filter(acl -> notMatch(acl, username))
            .collect(toList());
        curatorClient.setACL().withACL(acls).forPath(pathOps.makePath(znodePath));
    }
