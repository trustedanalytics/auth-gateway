...
public class ZookeeperAuthorizationTest {
    ...
    public void addOrganization_zkWorks_znodeAddedWithAdminAcl() throws Exception {
        zookeeperGateway.addOrganization(ORG_ID, ORG_NAME);
        verify(zkClient).createZnode(ORG_ID, ORG_NAME, ADMIN_USER, ZookeeperPermission.CRDWA);
    }
    ...
    public void addOrganization_zkFails_exceptionRethrown() throws Exception {
        doThrow(new Exception())
            .when(zkClient).createZnode(ORG_ID, ORG_NAME, ADMIN_USER, ZookeeperPermission.CRDWA);
        zookeeperGateway.addOrganization(ORG_ID, ORG_NAME);
        verify(zkClient).createZnode(ORG_ID, ORG_NAME, ADMIN_USER, ZookeeperPermission.CRDWA);
    }
    ...
    public void addUser_noOperation_invokedWithoutExceptions() throws Exception {
        zookeeperGateway.addUser("id", "name");
    }
    ...
    public void addUserToOrg_orgNotExist_exceptionRethrown() throws Exception {
        doThrow(new KeeperException.NoNodeException())
            .when(zkClient)
            .addUserToAcl(ORG_ID, USER, ZookeeperPermission.CRDW);
        zookeeperGateway.addUserToOrg(USER, ORG_ID);
        verify(zkClient).addUserToAcl(ORG_ID, USER, ZookeeperPermission.CRDW);
    }
    ...
    public void removeOrganization_orgExists_loggedInKrbThenAclRevoked() throws Exception {
        zookeeperGateway.removeOrganization(ORG_ID, ORG_NAME);
    ...
    public void removeUser_noOperation_invokedWithoutExceptions() throws Exception {
        zookeeperGateway.removeUser("id", "name");
    }
